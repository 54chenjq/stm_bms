...
void wakeup_idle(uint8_t total_ic,SPI_HandleTypeDef *hspi) //Number of ICs in the system
{
	for (int i =0; i<total_ic; i++)
	{
	   cs_low(CS_PIN);
	   spi_read_byte(0xff,hspi);//Guarantees the isoSPI will be in ready mode
	   cs_high(CS_PIN);
	}
}
				...
				)
{
	...
	cs_low(CS_PIN); // commented for testing
	spi_write_read(cmd, 4, data, (BYTES_IN_REG*total_ic),hspi);         //Transmits the command and reads the configuration data of all ICs on the daisy chain into rx_data[] array
	cs_high(CS_PIN);

	//test_func(cmd, 4, data, (BYTES_IN_REG*total_ic));
...

{
	...
	if (reg == 0)
	{
		...
		{
			data_counter = 0;
			LTC681x_rdstat_reg(stat_reg, total_ic,data,hspi);                            //Reads the raw status register data into the data[] array
			...
			{																		// current_ic is used as the IC counter
				...
				else if (stat_reg == 2)
				{
					...
					data_counter = data_counter +2; // might have to change to parsed_stat = data[data_counter] + (data[data_counter+ 1]<<8);
	...
	{
		LTC681x_rdstat_reg(reg, total_ic, data,hspi);
		...
		{																		  // current_ic is used as an IC counter
			...
			else if (reg == 2)
			{
				...
				parsed_stat = data[data_counter] + (data[data_counter+ 1]<<8);//Each start codes is received as two bytes and is combined to
...
uint8_t LTC681x_pladc(SPI_HandleTypeDef *hspi)
{
	...
	cs_low(CS_PIN);
	spi_write_array(4,cmd,hspi);
	adc_state = spi_read_byte(0xFF,hspi);
	cs_high(CS_PIN);
...
uint32_t LTC681x_pollAdc(SPI_HandleTypeDef *hspi)
{
	...
	{
		current_time = spi_read_byte(0xff,hspi);
		if (current_time>0)
		{
			finished = 1;
		}
	...
	for (int self_test = 1; self_test<3; self_test++)
	{
		...
		switch (adc_reg)
		{
		  case CELL:
			  ...
			  LTC681x_cvst(md,self_test,hspi);
			  LTC681x_pollAdc(hspi);
		  ...
		  case AUX:
			  ...
			  wakeup_idle(total_ic,hspi);
			  LTC681x_clraux(hspi);
			  LTC681x_axst(md,self_test,hspi);
			  LTC681x_pollAdc(hspi);

			  wakeup_idle(total_ic,hspi);
			  LTC681x_rdaux(0, total_ic,ic,hspi);
			  for (int cic = 0; cic < total_ic; cic++)
				{
		  ...
		  case STAT:
			  ...
			  LTC681x_statst(md,self_test,hspi);
			  LTC681x_pollAdc(hspi);

			  wakeup_idle(total_ic,hspi);
			  error = LTC681x_rdstat(0,total_ic,ic,hspi);
			  for (int cic = 0; cic < total_ic; cic++)
				{
									  ...
									  )
{
	...
	for (int self_test = 1; self_test<3; self_test++)
	{
		...
		switch (adc_reg)
		{
			case AUX:
			LTC681x_clraux(hspi);
			LTC681x_adaxd(adc_mode,AUX_CH_ALL,hspi);
			LTC681x_pollAdc(hspi);

			wakeup_idle(total_ic,hspi);
			error = LTC681x_rdaux(0, total_ic,ic,hspi);
			for (int cic = 0; cic < total_ic; cic++)
			{
				for (int channel=0; channel< ic[cic].ic_reg.aux_channels; channel++)
				{
			...
			case STAT:
			LTC681x_clrstat(hspi);
			LTC681x_adstatd(adc_mode,STAT_CH_ALL,hspi);
			LTC681x_pollAdc(hspi);
			wakeup_idle(total_ic,hspi);
			error = LTC681x_rdstat(0,total_ic,ic,hspi);
			for (int cic = 0; cic < total_ic; cic++)
								...
								)
{
	...
	{
	  wakeup_idle(total_ic,hspi);
	  LTC681x_adow(MD_26HZ_2KHZ,PULL_UP_CURRENT,CELL_CH_ALL,DCP_DISABLED,hspi);
	...
	{
	  wakeup_idle(total_ic,hspi);
	  LTC681x_adow(MD_26HZ_2KHZ,PULL_DOWN_CURRENT,CELL_CH_ALL,DCP_DISABLED,hspi);
	  conv_time =LTC681x_pollAdc(hspi);
	}
...
void LTC681x_run_gpio_openwire(uint8_t total_ic, // Number of ICs in the daisy chain
								...
								)
 {
	...
	{
	   wakeup_idle(total_ic,hspi);
	   LTC681x_adax(MD_7KHZ_3KHZ, AUX_CH_ALL,hspi);
	   conv_time= LTC681x_pollAdc(hspi);
	}
	...
	{
	   wakeup_idle(total_ic,hspi);
	   LTC681x_axow(MD_7KHZ_3KHZ,PULL_DOWN_CURRENT,hspi);
	   conv_time =LTC681x_pollAdc(hspi);
	}
